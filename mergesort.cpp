...
void MergeSort(std::vector<int>* numbers, int &SortComp, int &SortMacc) {
   MergeSortRecurse(numbers, 0, numbers->size() - 1, SortComp, SortMacc);
}
...
void MergeSortRecurse(std::vector<int>* numbers, int i, int k, int &SortComp, int &SortMacc) {
   ...
   if (i < k) {
      ...
      MergeSortRecurse(numbers, i, j, SortComp, SortMacc);
      MergeSortRecurse(numbers, j + 1, k, SortComp, SortMacc);
      
      // Merge left and right partition in sorted order
      Merge(numbers, i, j, k, SortComp, SortMacc);
   }
}
